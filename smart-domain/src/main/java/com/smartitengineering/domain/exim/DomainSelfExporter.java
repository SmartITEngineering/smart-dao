...
/**
 * As the name suggests the API indicates that a domain is able to export itself
 * and to determine for which MIME types it can it has an operation dedicated op
 * for it.
 * @author imyousuf
 * @since 0.4
 */
public interface DomainSelfExporter<T> {

    /**
     * It provides the operation for clients to determine whether the domain is
     * able to export itself to the MIME type the client wants.
     * @param mimeType Type to export to
     * @return True if and only if the mimeType is supported for export
     */
    public boolean isMIMESupported(String mimeType);

    /**
     * Exports the domain to the specified stream in specified type
     * @param mimeType Type to export to.
